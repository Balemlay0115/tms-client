...
export class EnrollmentFormComponent implements OnInit {
  ...
  form = this.fb.nonNullable.group({
    // In enrollment-form.ts form definition
    studentId: [
      '',
      [Validators.required, Validators.pattern(/^\d+$/)], // Change pattern to accept numbers only
    ],
    // Validates formats like MAT-101, CS-101, or CSE-101
    courseId: ['MAT-101', [Validators.required, Validators.pattern(/^[A-Z]{2,4}-\d{3}$/i)]],
  ...
  submit() {
    if (this.form.valid) {
      ...
      const studentIdValue = String(raw.studentId ?? '')
        .trim()
        .toUpperCase();

      // Flat payload matching your Scalar API definition
